<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-else-if="ruleForm.shangpinxiangqing" label="商品详情" prop="shangpinxiangqing" >
				<span class="text" v-html="ruleForm.shangpinxiangqing"></span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					url: `chongwuyongpinshangcheng/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						...
						this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../springbootio7q3t15/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
...
var objcross = this.$storage.getObj('crossObj');
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chongwuyongpinshangcheng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chongwuyongpinshangchengCrossAddOrUpdateFlag = false;
											this.parent.search();
