	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="宠物性别" prop="chongwuxingbie" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="求助内容" prop="qiuzhuneirong" >
					<el-input v-model="ruleForm.qiuzhuneirong" placeholder="求助内容" clearable  :readonly="ro.qiuzhuneirong"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="求助内容" prop="qiuzhuneirong" >
					<el-input v-model="ruleForm.qiuzhuneirong" placeholder="求助内容" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="date" v-if="type!='info'" label="领养时间" prop="jiuzhushijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.jiuzhushijian" 
						type="datetime"
						:readonly="ro.jiuzhushijian"
						placeholder="领养时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.jiuzhushijian" label="领养时间" prop="jiuzhushijian" >
					<el-input v-model="ruleForm.jiuzhushijian" placeholder="领养时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="领养记录" prop="jiuzhujilu" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="领养记录"
					v-model="ruleForm.jiuzhujilu" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jiuzhujilu" label="领养记录" prop="jiuzhujilu" >
				<span class="text">{{ruleForm.jiuzhujilu}}</span>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.jiuzhushijian = this.getCurDateTime()
		},
		methods: {
			// 下载
			download(file){
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='jiuzhujilu'){
							this.ruleForm.jiuzhujilu = obj[o];
							this.ro.jiuzhujilu = true;
							continue;
						}
						if(o=='jiuzhushijian'){
...
var objcross = this.$storage.getObj('crossObj');
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zhiyuanzhejiuzhu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zhiyuanzhejiuzhuCrossAddOrUpdateFlag = false;
											this.parent.search();
