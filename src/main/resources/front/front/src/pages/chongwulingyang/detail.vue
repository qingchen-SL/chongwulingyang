	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/chongwulingyang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">领养申请</div>
						<div class="text "  >{{detail.lingyangshenqing}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('chongwulingyang','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('chongwulingyang','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('chongwulingyang','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
            <el-button class="reserveBtn"
                       v-if="btnAuth('chongwulingyang','志愿者领养')"
                       @click="onAcross('chongwulingyang','是','','','')" type="warning">
              志愿者领养
            </el-button>
            <el-button class="reserveBtn"
                       v-if="btnAuth('chongwulingyang','用户领养')"
                       @click="onAcross('chongwulingyang','是','','','')" type="warning">
              用户领养
            </el-button>
					</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/chongwulingyang', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/chongwulingyangAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此宠物领养？') .then(_ => {
					this.$http.post('chongwulingyang/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
