	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiuzhuxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">宠物性别</div>
						<div class="text "  >{{detail.chongwuxingbie}}</div>
					</div>
					<div class="item">
						<div class="lable">宠物年龄</div>
						<div class="text "  >{{detail.chongwunianling}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">求助内容</div>
						<div class="text "  >{{detail.qiuzhuneirong}}</div>
					</div>
					<div class="item">
						<div class="lable">救助地点</div>
						<div class="text "  >{{detail.jiuzhudidian}}</div>
					</div>
					<div class="item">
						<div class="lable">求助人</div>
						<div class="text "  >{{detail.qiuzhuren}}</div>
					</div>
					<div class="item">
						<div class="lable">求助时间</div>
						<div class="text "  >{{detail.qiuzhushijian}}</div>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiuzhuxinxi','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiuzhuxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jiuzhuxinxi','志愿者救助')"
							@click="onAcross('zhiyuanzhejiuzhu','是','','','')" type="warning">
							志愿者救助
						</el-button>

						<el-button class="auditBtn" v-if="btnAuth('jiuzhuxinxi','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`jiuzhuxinxi`);
			...
			backClick(){
				...
				this.$router.push({path: '/index/jiuzhuxinxi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiuzhuxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此救助信息？') .then(_ => {
					this.$http.post('jiuzhuxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
