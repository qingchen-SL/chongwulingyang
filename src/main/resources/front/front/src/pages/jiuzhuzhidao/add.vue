<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="领养流程" prop="jiuzhuliucheng">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="领养流程"
			...
			<el-form-item class="add-item" label="领养指导" prop="jiuzhuzhidao">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="领养指导"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiuzhuzhidao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiuzhuzhidao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
