<template>
	<div :style='{"padding":"0 0 20px","margin":"0px auto","color":"#666","background":"#fff","width":"1200px","fontSize":"16px","position":"relative"}'>
		...
		<div class="section-title" :style='{"margin":"10px 0","color":"#333","textAlign":"center","background":"#f7db61","width":"100%","fontSize":"24px","lineHeight":"50px","border-bottom":"1px solid #1f292f"}'>我的订单</div>
							...
							if(item.type==1||item.type==3) {
								...
								this.$http.post(table+'/update', this.user).then(res => {
									...
									this.$http.post(`orders/update`, item).then(res => {
										if(item.tablename == `chongwuyongpinshangcheng`){
											// 加回库存数量
											good.alllimittimes = good.alllimittimes + item.buynumber;
											this.$http.post(`chongwuyongpinshangcheng/update`, good).then(res => {
											});
										}
										this.$message({
