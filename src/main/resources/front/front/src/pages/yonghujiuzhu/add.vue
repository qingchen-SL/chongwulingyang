<template>
	<div class="add-update-preview">
      ...
      <el-form-item class="add-item"  label="领养状态" prop="lingyangzhuangtai">
        <el-select v-model="ruleForm.lingyangzhuangtai" placeholder="请选择领养状态" :disabled=" false  ||ro.lingyangzhuangtai" >
          <el-option
      ...
      <el-form-item class="add-item" label="宠物详情" prop="chongwuxiangqing">
        <editor
    ...
    async onSubmit() {
      await this.$refs["ruleForm"].validate(async valid => {
        if(valid) {
          ...
          await this.$http.post(`chongwuxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
            if (res.data.code == 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
