	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhiyuanzhejiuzhu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">领养记录</div>
						<div class="text "  >{{detail.jiuzhujilu}}</div>
					</div>
					<div class="item">
						<div class="lable">领养时间</div>
						<div class="text "  >{{detail.jiuzhushijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhiyuanzhejiuzhu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhiyuanzhejiuzhu','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/zhiyuanzhejiuzhu', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zhiyuanzhejiuzhuAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此志愿者领养？') .then(_ => {
					this.$http.post('zhiyuanzhejiuzhu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
